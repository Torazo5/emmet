...
leopard = pvleopard.create(access_key=os.getenv("PICO_ACCESS_KEY"))
print('code starting')
...
# Create Adafruit_NeoPixel object with appropriate configuration.
strip = Adafruit_NeoPixel(LED_COUNT, LED_PIN, LED_FREQ_HZ,LED_DMA,LED_INVERT,LED_BRIGHTNESS,LED_CHANNEL)
strip.begin()
time.sleep(1)  # Allow time for initialization
print('code testing through led init')
def turn_off_leds():
...
def audio(prompt):
    ...
    subprocess.run(['mpg321', '-a', 'hw:0,0', 'output_audio.wav'])
    ...
    try:
        while True:
            ...
            if porcupine_keyword_index >= 0:
                ...
                for x in range(LED_COUNT):
                    strip.setPixelColor(x, Color(0, 0, 255))  # Blue color for listening
                strip.show()
                break
...
def main():
    try:
        while True:
            ...
            # for x in range(LED_COUNT):
            #     strip.setPixelColor(x, Color(255, 165, 0))  # Orange color for processing
            # strip.show()
