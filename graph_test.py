...
def save_recall_memory(memories: List[KnowledgeTriple], config: RunnableConfig) -> str:
    ...
    for memory in memories:
        ...
        print(f"Memory prepared: {memory}")  # Logging prepared memory

    # Add texts to the vector store
    recall_vector_store.add_texts(texts=texts, metadatas=metadatas, ids=ids)
    print("Memories saved and persisted.")
...
def search_recall_memories(query: str, config: RunnableConfig) -> List[str]:
    ...
    user_id = get_user_id(config)
    # Define the filter for user_id
    filter = {"user_id": user_id}

    documents = recall_vector_store.similarity_search(query, k=3, filter=filter)
